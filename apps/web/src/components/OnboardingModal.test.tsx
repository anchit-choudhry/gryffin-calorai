...
describe("OnboardingModal", () => {
  ...
  it("renders step 0 welcome screen", () => {
    ...
    expect(screen.getByText(/Offline-first/i)).toBeTruthy();
  });

  it("renders step 1 with body basics fields", () => {
    renderModal({ step: 1 });
    expect(screen.getByText("Set up your goals")).toBeTruthy();
    expect(screen.getByLabelText(/Age/i)).toBeTruthy();
  });

  it("renders step 2 with activity level options", () => {
    renderModal({ step: 2 });
    expect(screen.getByText(/How active are you/i)).toBeTruthy();
  });

  it("renders step 3 with goal options", () => {
    renderModal({ step: 3 });
    expect(screen.getByText(/primary goal/i)).toBeTruthy();
  });

  it("renders step 4 review with Save button", () => {
    renderModal({ step: 4 });
  ...
  it("shows 'Saving...' when isLoading is true on last step", () => {
    renderModal({ step: 4, isLoading: true });
  ...
  it("shows step indicator dots", () => {
    renderModal({ step: 0, totalSteps: 5 });
    expect(screen.getByText("Set up your goals")).toBeTruthy();
  });

  it("shows unit toggle buttons on step 1", () => {
    renderModal({ step: 1 });
    expect(screen.getByRole("button", { name: /kg/i })).toBeTruthy();
    expect(screen.getByRole("button", { name: /lb/i })).toBeTruthy();
  });

  it("clicks female sex button on step 1", async () => {
    renderModal({ step: 1 });
  ...
  it("changes age input on step 1", async () => {
    renderModal({ step: 1 });
  ...
  it("clicks height unit toggle on step 1", async () => {
    const mockSetLength = vi.fn();
    renderModal({ step: 1, setLengthUnit: mockSetLength });
  ...
  it("converts height when unit toggle is clicked after input", async () => {
    const mockSetLength = vi.fn();
    renderModal({ step: 1, lengthUnit: "cm", setLengthUnit: mockSetLength });
  ...
  it("clicks weight unit toggle on step 1", async () => {
    const mockSetWeight = vi.fn();
    renderModal({ step: 1, setWeightUnit: mockSetWeight });
  ...
  it("converts weight when unit toggle is clicked after input", async () => {
    const mockSetWeight = vi.fn();
    renderModal({ step: 1, weightUnit: "kg", setWeightUnit: mockSetWeight });
  ...
  it("clicks an activity level button on step 2", async () => {
    renderModal({ step: 2 });
  ...
  it("clicks a goal button on step 3", async () => {
    renderModal({ step: 3 });
  ...
  it("Get started button on step 0 calls nextStep", async () => {
  ...
  it("Continue button on step 3 calls nextStep after validation", async () => {
    const mockNext = vi.fn();
    renderModal({ step: 3, nextStep: mockNext });
    const continueBtn = screen.getByRole("button", { name: /Continue/i });
    await act(async () => {
      fireEvent.click(continueBtn);
    });
  ...
  it("renders step 1 with lb weight unit showing lb placeholder", () => {
    renderModal({ step: 1, weightUnit: "lb" });
    expect(screen.getByPlaceholderText("154")).toBeTruthy();
  });

  it("renders step 1 with in length unit showing in placeholder", () => {
    renderModal({ step: 1, lengthUnit: "in" });
    expect(screen.getByPlaceholderText("67")).toBeTruthy();
  });

  it("renders step 4 review with dashes when BMR cannot be computed", () => {
    ...
    renderModal({ step: 4, form: emptyResult.current });
