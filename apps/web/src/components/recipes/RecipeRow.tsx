      ...
      <li className="group flex justify-between items-start p-4 hover:bg-paper-muted transition">
        ...
        <div className="flex items-center gap-1 ml-4 flex-shrink-0 opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 transition-opacity">
          <button
            ...
            className={iconBtn}
          >
            <Share2 className="size-3.5 text-ink-soft" aria-hidden="true" />
          </button>
          <button
            type="button"
          ...
          <button
            ...
            className={iconBtn}
          >
            <X className="size-3.5 text-ink-soft hover:text-persimmon" aria-hidden="true" />
          </button>
        </div>
      </li>
      {shareOpen && <RecipeShareSheet recipe={recipe} onClose={() => setShareOpen(false)} />}
    </>
  );
}
