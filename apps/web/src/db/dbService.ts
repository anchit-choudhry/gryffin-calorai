...
// 18. Version 18: nutritionData optional field added to foodItems, recurringMealFoods,
// and mealTemplateFoods (feature 14 - Micronutrient Tracking). No index changes needed.
...
export interface TableConflictEntry {
  incoming: number;
  existing: number;
}

export interface ConflictSummary {
  foodItems: TableConflictEntry;
  recipes: TableConflictEntry;
  waterLogs: TableConflictEntry;
  bodyMeasurements: TableConflictEntry;
  stepLogs: TableConflictEntry;
  activityLogs: TableConflictEntry;
  fastingSessions: TableConflictEntry;
}
...
export const detectConflicts = async (
  ...
  const [
    ...
    existingFastingSessions,
  ] = await Promise.all([
    foodItems.where("userId").equals(userId).count(),
    recipes.where("userId").equals(userId).count(),
    waterLogs.where("userId").equals(userId).count(),
    bodyMeasurements.where("userId").equals(userId).count(),
    stepLogs.where("userId").equals(userId).count(),
    activityLogs.where("userId").equals(userId).count(),
    fastingSessions.where("userId").equals(userId).count(),
  ]);

  return {
    foodItems: { incoming: t.foodItems.length, existing: existingFoodItems },
    recipes: { incoming: t.recipes.length, existing: existingRecipes },
    waterLogs: { incoming: t.waterLogs.length, existing: existingWaterLogs },
    bodyMeasurements: { incoming: t.bodyMeasurements.length, existing: existingBodyMeasurements },
    stepLogs: { incoming: t.stepLogs.length, existing: existingStepLogs },
    activityLogs: { incoming: t.activityLogs.length, existing: existingActivityLogs },
    fastingSessions: { incoming: t.fastingSessions.length, existing: existingFastingSessions },
  };
};
