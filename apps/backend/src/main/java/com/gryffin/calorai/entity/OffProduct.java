 ...
 * and is never written to by the Spring application. {@code @Immutable} prevents Hibernate from
 * generating UPDATE statements. The {@code search_vec} TSVECTOR column is intentionally unmapped;
 * it is managed by a PostgreSQL trigger and queried via native SQL only.
 */
@Entity
@Immutable
