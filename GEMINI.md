# Project Documentation: Gryffin Calorai (v0.9.0 - UX Uplift & AI Scaffold)
...
- **State Management:** Global state is managed by a single Zustand store
  (`apps/web/src/state/AppState.ts`), composed from 9 functional slices (foodSlice, recipeSlice,
  bodySlice, activitySlice, trackerSlice, settingsSlice, coreSlice, syncSlice, uiSlice) unified in
  AppState.ts.
- **Persistence:**
  - **Client:** IndexedDB via Dexie.js 4 (currently **schema version 20**) with compound
    indices for performance.
  - **Server:** PostgreSQL 18 with migrations managed by Flyway.
- **Cloud Sync:** Bidirectional push-then-pull sync via delta changes endpoints (
  `GET /changes?since=`)
...
- **State:** Zustand 5 (composed of 9 functional slices)
- **Database:** IndexedDB via Dexie.js 4 (v20 schema)
...
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **State**             | `apps/web/src/state/AppState.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Single Zustand store; all mutations here; composed of 9 slices: foodSlice, recipeSlice, bodySlice, activitySlice, trackerSlice, settingsSlice, coreSlice, syncSlice, uiSlice                                    |
...
| **Motion**            | `apps/web/src/lib/motionVariants.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Shared page, section, coachmark, spotlight, arrow variants; `useSectionMotion()` returns crossfade under reduced-motion; `useHeroSection()`, `heroVariants`, `counterPopVariants`, `easeSpring`                 |
...
| **Tests**             | `apps/web/src/**/*.test.{ts,tsx}` (112 test files, 2080 tests)                                                                                                                                                                                                                                                                                                                                                                                                                               | Vitest + jsdom + fake-indexeddb + coverage                                                                                                                                                                      |
...
- `state/AppState.ts`: Central Zustand store. Composed from `foodSlice`, `recipeSlice`, `bodySlice`,
  `activitySlice`, `trackerSlice`, `settingsSlice`, `coreSlice`, `syncSlice`, and `uiSlice`.
- `db/dbService.ts`: Dexie.js service layer. Defines schema v20, compound indices, and CRUD
  abstractions.
...
- `dashboard/`: Sub-components (`DashboardHero`, `DateKicker`, `EditorialFrame`, `LogEntry`,
  `MacroStat`, `SectionHeader`, `DailyVitalsStrip`, `RuleTicks`).
...
- `FoodLogger.tsx`, `VoiceFoodLogger.tsx`, `BarcodeScanner.tsx`, `PhotoFoodLogger.tsx`: Nutrition
  logging tools.
- `WaterTracker.tsx`, `StepTracker.tsx`, `FastingTimer.tsx`, `DailyVitalsStrip.tsx`: Activity and
  habit tracking.
...
- `SyncStatusChip.tsx`, `CloudSyncPanel.tsx`, `HarvestStamp.tsx`, `EmptyState.tsx`,
  `QuickAddModal.tsx`, `FoodSearchCombobox.tsx`, `ProvenanceBadge.tsx`, `PhotoStrip.tsx`: Sync,
  achievements, navigation, search, and UX.
...
- `useProgressData.ts`, `useWeeklySummary.ts`, `useStreaks.ts`, `useMicronutrientData.ts`: Data
  aggregation.
- `useSyncService.ts`, `useBarcodeScanner.ts`, `useMotionPreset.ts`, `useRecentFoods.ts`: Service
  and UX hooks.
...
- `utils.ts`: Utility functions (cn, date formatting, log grouping, `shiftISODate`).

### Page Components (`apps/web/src/pages`)

- `Dashboard.tsx`: Main overview with editorial layout and Today/Week diary tabs.
- `Recipes.tsx`: Recipe management interface.
- `Progress.tsx`: Data-driven visualizations (Calorie Trend, Macros, Micronutrients, etc.).
- `Settings.tsx`: User profile, TDEE management, display settings, and data settings.
...
- **Naming Conventions:**
  ...
  - **Branded Types:** Use branded types for all IDs (UserId, FoodItemId, RecipeId, WaterLogId,
    BodyMeasurementId, UserAchievementId, StepLogId, FastingSessionId, ActivityLogId, ISODate,
    FoodPhotoId) to
    prevent ID mix-ups.
  - **Zustand Store:** Composed slices in `apps/web/src/state/slices/` (9 slices total). No local
    component state
...
- [x] Cloud Sync (bidirectional offline-first sync) - **Web and Backend Complete**
- [x] Backend Security Hardening (OWASP + Google Java Style, Javadoc, Rate limiting)
- [x] UX Uplift (typography, ornaments, quick-add, dashboard, streamlined logger, photo logging
  scaffold, retention rituals, navigation, logging speed, system polish, onboarding, progress
  visualization) - **Complete**
- [ ] AI Photo Logging (vision LLM proxy) - **Planned**
- [ ] Platform personalization (PWA, haptics, accent theming, Settings IA) - **Planned**
- [ ] Social / Sharing - **Planned**
- [ ] Native iOS/Android apps (v0.9.0-v1.0.0) - **Planned**
