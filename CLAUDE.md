...
- **Store:** Zustand store composed from 9 slices (`foodSlice`, `recipeSlice`, `bodySlice`,
  ...
  `density: Density` (persisted to localStorage as `gc_density`); `syncSlice.pendingSyncCount:
  number` (shown in `SyncStatusChip` when syncing).
...
**Last Updated:** June 8, 2026 | **Current release:** v0.8.0 (released June 7, 2026) | **In
progress:** v0.9.0 - UX uplift WS A-F complete + WS G-J partial (uncommitted, 107 test files, 2025
tests); code simplification pass (9 fixes); B5 AI Photo Logging backend; UX uplift WS K-M; B6
social; TC1-TC4 test debt
