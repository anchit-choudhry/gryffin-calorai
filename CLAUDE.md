...
- `CLAUDE.md` - context line, footer (`Last Updated` / `Current release` / `In progress`)
...
- See @@.claude/rules/ux-principles.md for complete rules
...
`.claude/skills/` (dexie-migration, flyway-migration, scaffold-backend,
...
For feature requirements, API, and non-functional requirements, see
...
| **AI classifier**  | `apps/web/src/lib/localFoodClassifier.ts`                      | Wraps `@huggingface/transformers` pipeline; singleton with reset-on-failure pattern (`pipelinePromise = null` in catch); `classifyFood(imageFile)` returns sorted `RecognizedFoodItem[]`                                                                   |
| **AI logging API** | `apps/web/src/lib/aiLoggingApi.ts`                             | `recognizePhoto(file)` + `parseText(text)` - both delegate to `classifyFood`; per-item error isolation via `continue` so one bad result doesn't abort the set                                                                                              |
...
**Last Updated:** June 28, 2026 | **Current release:** v0.21.0 (released June 2026) | **In
progress:** v0.22.0 (146+ test files, 2788+ tests)
