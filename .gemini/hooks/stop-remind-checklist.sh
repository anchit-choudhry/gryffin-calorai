#!/usr/bin/env bash
# Runs when Gemini's turn ends (Stop event).
# If any apps/web/src/ TypeScript files were modified this session,
# prints the pre-commit checklist from GEMINI.md as a reminder.
set -uo pipefail
...
echo ""
echo "Modified web source files detected. Pre-commit checklist (GEMINI.md):"
